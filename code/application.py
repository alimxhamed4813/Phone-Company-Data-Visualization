...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event_data in log['events']:
        ...
        # if the billing month has changed, we must update it
        # to the current month
        if curr.month != billing_month:
            new_month(customer_list, curr.month, curr.year)

        if event_data["type"] == "call":
            # creating call object with attributes from keys
            new_call = Call(event_data["src_number"],
            ...
            customer = find_customer_by_number(event_data["src_number"],
                                               customer_list)
            if customer:
                customer.make_call(new_call)

            # if it's an incoming call
            customer = find_customer_by_number(event_data["dst_number"],
                                               customer_list)
            if customer:
                customer.receive_call(new_call)
